...
- **Per-category unread state cap** (`UNREAD_CAP_PER_CATEGORY = 300` in `lib/queries.ts`). After every poll cycle, `pruneUnreadCategoryCaps` finds unread items beyond rank 300 in each category (by `published_at DESC`) and sets `read_at` on them. `consumed_at` is NEVER set by the prune, so /read history is unaffected. This is the "finite, not infinite" mission invariant — without it, sources with years of backlog (e.g. 5500+ podcast episodes) make the queue feel infinite even when display is capped. Pruned items are "pending": still in `items`, with `item_state.read_at` set, invisible to every view. They never re-surface unless the cap is raised AND the rank logic finds them again (it won't, because new items always rank higher).
- **All view = quota-based sampling + stride interleaving + mild surprise.** `ALL_VIEW_QUOTAS` in `lib/queries.ts` defines a per-category quota; `getMainFeedItems` runs a two-phase algorithm: (1) for each category, pull a slightly oversampled recency window from SQL (`SURPRISE_POOL_MULTIPLIER` = 1.5x quota by default), then narrow to quota via weighted sampling biased toward newer items (`SURPRISE_RECENCY_BIAS`); (2) interleave the per-category lists via stride scheduling (`interleaveByQuota`) so high-quota categories appear more often but no category clumps two-in-a-row, with `INTERLEAVE_JITTER` for mild perturbation so the cadence isn't perfectly mechanical. With current quotas {podcasts:100, music/film/reading/bluesky:50}, the deterministic pattern (jitter=0) is `P M F P R B` repeating — podcasts every other slot, the others rotating in between. Sum of quotas = the All view's max size (currently 300). High-signal items are NOT pinned to the top and the output is **NOT sorted by `published_at`** — that strict recency sort was exactly what made categories clump whenever a source published in a tight time window. The downstream `groupByDate` is order-preserving so the interleave pattern survives the Today/Yesterday/etc. split. All knobs (quotas + the three SURPRISE/INTERLEAVE constants) are at the top of `lib/queries.ts`.
- **`MAIN_FEED_LIMIT = 300`** in `app/page.tsx` and `FEED_LIMIT = 300` in `components/FeedClient.tsx` are the client-side ceiling — keep them in sync. The `/api/items` endpoint caps `limit` at 500 as a defensive ceiling. The All view's actual size is determined by quotas, not the limit, so if the quotas sum to 250 the All view returns 250 items even with `limit=300`.
- **Category-filtered views use pure recency** (`ORDER BY published_at DESC`), not the quota logic — see `app/api/items/route.ts`. Each category tab shows up to 300 items (the per-category cap). Music/film/reading rarely hit the cap; podcasts and bluesky usually do.
- **All count is the actual cardinality of the All view, not the grand total of unread items.** `getCategoryCounts` returns `all = sum(min(quota, unread_count))` across categories. So the number on the All tab matches what you actually see when you tap it, instead of advertising 600 items but only showing 267. Per-category counts are the literal unread totals (already bounded by the prune).
...
- **Bulk dismiss is NOT "marked as read"** (`POST /api/items/read-all`, called by the "That's enough for now." button at the bottom of the feed). The handler `markAllUnreadAsRead` in `lib/queries.ts` only sets `read_at`, never `consumed_at`. Bulk-dismissed items do NOT appear in `/read`. Same invariant applies to `pruneUnreadCategoryCaps` — auto-pruning is conceptually a bulk dismiss. Don't conflate the two — a future change that bulk-sets `consumed_at` would dump thousands of items the user never opened into the history view.
