...
- **Silent background refresh on PWA resume.** When `document.visibilitychange` fires and more than 60s has elapsed since the last manual refresh, `FeedClient` posts to `/api/refresh`, re-fetches counts, diffs item IDs, and — if new items arrived — surfaces a "Load now" toast. It deliberately does NOT replace the visible list or reset `focusedIndex` until the user taps the toast, so resuming the app never yanks the scroll position.
