...
npm run check:hoops:movers       # Team-page "what moved" gate: the biggest three are the biggest three,
                                 #   the sums and the mix identity hold, an abstained team is never
                                 #   explained, and real box scores anchor Giannis/Tatum (same)
...
npm run check:books:health       # Books file-health gate: clean epub → zero findings, each damage pattern → exactly its finding,
                                 #   the language check anchored on 11 real Gutenberg books, health caching never touches the file (same)
...
npm run check:feed               # Feed gate: a dismissed Verge review stays dismissed when the reviews feed picks it up later
                                 #   (state moves to the winning copy; matched by post id, not URL), retention never resurrects
                                 #   what a deep feed still carries (tombstones), never-seen items survive retention, dead ids
                                 #   can't wedge the dismiss outbox, and the client pins (outbox-before-refetch, keepalive) (same)
```

No test suite. `npm run build` is the type-check gate — always run it before pushing. It is
**also** the data and engine gate: `prebuild` runs every `check:hoops*` and `check:books*` script plus `check:feed`, so a stale
...
inverted, or a dismissed article that comes back on the next crawl, fails the build (and the Railway deploy) rather than shipping. See "Hoops" below.
...
node scripts/inspect.mjs dedup                   # cross-source duplicates, Verge post-id dups, tombstones, resurrection signature (zero rows = healthy)
```
...
2. The poller calls `fetchAllSources()` (`lib/fetcher.ts`) on an interval (`app.poll_interval_minutes` in `config/feeds.yml`, default 15), which re-reads `config/feeds.yml` each cycle, syncs the `sources` table, fetches new content, runs the cross-source dedup, **auto-prunes any sources/items removed from config**, runs `pruneExpiredUnread`, and calls `checkReleaseNotifications` (which has its own once-per-local-day guard)
...
- `dismissed_keys` — tombstones keyed `(source_id, external_id)` with a `reason`: `dismissed` (retention hard-deleted a silently-dismissed row) or `folded` (the cross-source dedup deleted this copy in favour of the same piece under another source). Both fetchers skip any key present here, which is what stops a deep feed from re-inserting a deleted row as a brand-new unread item. Rows leave with their source.
...
- **Time-based falloff** (`UNREAD_TTL_HOURS` in `lib/queries.ts`). After every poll cycle, `pruneExpiredUnread` marks RSS items older than their category's TTL as read. RSS categories (music, books, film, tech_review, podcasts, reading) have a 7-day TTL. Bluesky uses a **position-based window** (`BSKY_WINDOW`) instead of a TTL — see the All view invariant below. `consumed_at` is NEVER set by the prune, so /read history is unaffected. Pruned items are "pending": still in `items`, with `item_state.read_at` set, invisible to every view. A separate universal retention step (`READ_RETENTION_HOURS`, 7 days) then **hard-deletes** any item where `read_at` is set but `saved_at` and `consumed_at` are both null — i.e., silently dismissed items that the user never engaged with. Saved items and opened items live forever so `/saved` and `/read` are complete histories. 🔴 **Deleting the row also deletes the memory that it was dismissed, and the Verge reviews feed runs ~3 weeks deep (podcast feeds carry every episode ever)** — so before 2026-09-04 every dismissed review came back as a fresh unread row, under a new id, on the first crawl after its 7th day, and was counted in the "N new items" toast. Now every row retention deletes leaves a `dismissed_keys` tombstone that `fetchRssSource`/`fetchBlueskySource` honour. 🔴 **Retention deletes only rows whose STATE says dismissed.** The old step also dropped every item with no state row at all that was older than the window — i.e. every never-seen unread item in any category whose TTL is longer than retention (one Tune knob away), re-minted under a new id every crawl. `check:feed` pins both.
...
- **Cross-source dedup** (`dedupCrossSourceDuplicates` in `lib/fetcher.ts`, after the RSS fetches, every cycle). Two known overlaps: `verge-full` (`reading`) and `verge-reviews` (`tech_review`) both carry a review, and the review copy wins (higher priority); and The Verge posts every Vergecast episode as an article under `/podcast/`, which the `vergecast` podcast row wins (it opens in Apple Podcasts; matched on title). 🔴 **The reviews feed is a tag applied AFTER publication, so the full-feed copy lands first** and may already be dismissed, saved or opened when the review copy arrives. Whatever the user did to the losing row MOVES to the winner (`read_at`/`saved_at`/`consumed_at`/`notes`, winner's own values kept) before the loser is deleted — the old dedup deleted the full-feed row AND its `item_state`, which is exactly how a dismissed review came back unread in Tech Review and a saved one vanished from `/saved`. 🔴 **Matching is by WordPress post id** (`vergePostId`: guid `?p=N`, the `/N/auto-draft` guid variant, or `/N/` in the link path), not by URL — the section prefix and slug get edited after publication and the items upsert never rewrites `url`, so exact-URL matching missed exactly the rows that had been touched. The loser's key is tombstoned `folded` so the full feed doesn't re-insert (and re-count) it every crawl for the two days it still carries it; a review whose full-feed twin was tombstoned `dismissed` is born read.
...
- **Dismissals are durable — the outbox** (`lib/dismiss-outbox.ts`). Every dismiss, clear-above, dismiss-visible and undo is written to localStorage as a batch first, posted to `/api/items/read-bulk` with `keepalive: true`, and removed only on a 2xx; leftovers replay on mount, on `online`, and at the START of every refresh (resume, pull, `r`) — before the crawl and the refetch. 🔴 The failure this closes: on the phone the dismiss request is the last thing before the app is put away, iOS suspends a backgrounded PWA fast enough to kill an in-flight fetch, the card had already vanished optimistically, and the resume refresh (which applies silently when you're at the top — exactly where clear-above leaves you) brought the article straight back. FIFO, so a dismiss and its undo replay in order; the server ignores ids that no longer exist (`markReadBulk`) so one dead id can't wedge the queue. Open/save also send with `keepalive`. 🔴 **A client-side return to the feed refetches** (`feedMountedThisDocument` in `FeedClient.tsx`): Next's router cache can mount `/` from an RSC payload up to 30s old, from before the last dismissals, so the SSR list is not trusted on a remount.
...
- `POST /api/items/[id]/read` — marks read (upserts `item_state.read_at`). Kept for scripts; FeedClient's per-card dismiss now rides `read-bulk` through the outbox.
...
- `POST /api/items/read-bulk` — body `{ ids: string[], unread?: boolean }`. Bulk mark-read or bulk-clear-read in a single transaction (`markReadBulk`). Backs every FeedClient dismissal and undo via the outbox, which replays a batch until it lands — so it is idempotent and **ignores ids that no longer exist** rather than failing the batch on `item_state`'s foreign key.
...
identity" (byte identity, §4) — with ONE exception, `language-mismatch`, which a metadata edit
fixes for free (`lib/books/healthRemedy.ts`'s `METADATA_FIXABLE` / `needsReplacementFile`; the
card's footer is gated on it, so a wrong two-letter tag never tells you to re-download a
perfectly good book). The report is cached in `books.health_json` — computed at ingest,
...
pinned only at their boundaries. 🔴 The language half is the exception and has to be: text built
from the detector's own word lists would verify itself, so it runs against
`books-fixture/language-samples.json` — real passages from **11 Project Gutenberg books in seven
languages, labelled by Gutenberg's own catalogue header**, regenerated by
`scripts/gen-language-samples.mjs` (network needed to regenerate; the gate only reads the JSON).
Falsification-tested **8/8** originally (ADEPT suppression removed,
...
plus **14/15** for the language check — the 15th (folding diacritics before scoring) is
deliberately UNCAUGHT and was the useful one: it proved an overclaim in the source comment, since
folding costs 0.2–3.1 points of the winner's rate and changes no verdict on any of the 11 books.
...
The library is English, and a file that isn't is invisible to every other check in the
report: it unzips, its spine resolves, it has plenty of prose and a full TOC, and it reads as a
perfectly healthy book right up until you open it on the device. `lib/books/language.ts` measures
the language of the TEXT and compares it with what the file CLAIMS. Two findings, at most one per
book: **`not-english`** (info — a fact about a library that is otherwise entirely English) and
**`language-mismatch`** (amber — the text and `dc:language` disagree, and the device picks
hyphenation and dictionary lookup from the tag).

🔴 **The sharper reason it exists is the OCR gate.** The English-only signals (the scanno list,
the stray-single-letter test) used to be gated on the DECLARED tag, which is exactly what a
mislabelled file gets wrong, and it failed in both directions: measured on the gate's own Don
Quijote passage, a Spanish novel tagged `en` scores **288 stray single letters** — every ordinary
"y" and "o" in the book — and was reported as OCR-damaged; an English novel tagged `de` silently
skipped the scanno list that would have caught its real damage. The gate is now the MEASURED
language, with the declared tag only as the fallback when there is too little text to measure, and
`check:books:health` pins both directions (asserting the Spanish passage really does carry the
signal before asserting it is not reported, so the case can't pass for the wrong reason).

**How it measures, and why not a dependency:** the share of tokens that are FUNCTION WORDS. They
are the most frequent words in any language, closed-class so the list can't go stale, and their
share is stable across authors and centuries — measured on the fixture, real English prose runs
**50–54%** and every other language scores **1.4–6.3%** on the English list. A language-detection
package would be more precise on a single sentence and buy nothing on a book. Non-Latin scripts
(Cyrillic, Greek, CJK, Arabic, Hebrew, Devanagari, Hangul, Thai) are caught by ALPHABET first,
before any word is counted.

🔴 **It refuses to guess, in four separate places, and each is pinned.** Under
`MIN_WORDS_FOR_VERDICT` (200) there is no verdict at all. A near-tie is no verdict: `MARGIN`
(1.5×) is what makes a half-English, half-Dutch book return *nothing* rather than a coin toss —
measured, a 40/60 mix scores 25.0% and 26.3%, a ratio of 1.05. A text that matches no known
language is reported as "does not read as English" but is **never NAMED**. And 🔴 **an accusation
requires a name**: the amber `language-mismatch` fires only when the check can say what the text
actually IS, so unnameable text tagged `en` gets the info line stating its number, never "tagged
English, reads as some other language" — a claim no measurement here supports, and far more
likely to mean damaged text than a lying tag. The repo's own `books-fixture/fixture.epub` (180k
words of `word0 word1 …`, tagged `en`) is the gate's case for it. `tagLanguageCode` maps only the
seven languages this check can actually measure, so an unrecognised tag (`ru`, `ja`) never becomes
an accusation either.

Known limits, stated rather than smoothed: a bilingual book is scored on its dominant language (or
gets no verdict, per the margin); the sample is STRIDED across the whole text, not taken from the
front, precisely so an English copyright page can't outvote the novel; and the seven-language
list is the seven it can name, not the seven that exist.
...
the To Read shelf. File health: `health.ts` (**pure** — the checker), `language.ts` (**pure**, imports
NOTHING — the function-word language detector; asserted, the `pages.ts` lesson),
`healthRemedy.ts` (**pure**, imports NOTHING — which finding a metadata edit fixes, read by the
client card) + `healthData.ts` (the cached
...
`gen-language-samples.mjs` (refetches the real Gutenberg passages the language check is anchored
on; FAILS if Gutenberg no longer reports the language it expects, rather than silently recording
the new label), `migrate-stump-progress.mjs` (one-time; carries `koreader_progress`+`end_percentage` only, ignores
...
**The pricing fixture (#24's last section) is LIVE and asserting.** `lib/hoops/pricing.ts` is a pure, DB-free TS port of hoops-sim's `allocate_minutes`/`allocate_minutes_with_absorption`/`allocate_minutes_depth_chart`/`raw_team_strength`/`team_net_rating` (read from `~/Code/hoops-sim`'s `src/hoops/minutesmodel.py`/`rosterratings.py`/`availability.py`, not modified — that repo is out of scope), reading every constant by name off `PricingConstants` (no hardcoded value). `scripts/check-hoops-pricing-fixture.ts` (in `prebuild`) prices every case in `hoops-data/hoops_pricing_fixture.json` through that module and demands hoops-sim's own answer to 1e-6. A missing fixture now FAILS the build rather than printing a note, because the sender has shipped it. 🔴 The JSON schema is this receiver's OWN design (documented in the script's header) since #24 names the required cases but not a byte-exact shape the way `hoops_fixture.json` already has. 🔴 `net`/`off`/`def` here are RAW/uncentered — `league_mean_raw` is deliberately never sent over the wire (kad-air/keith-hub#66), so this module can only ever compute a raw strength or a delta, never an absolute recentered rating.

#### Hub v2 — the Split, the Depth Chart and nightly strength (2026-09-02)

Three things the site had never had, all gated on `contract.features`, all taught here BEFORE hoops-sim was allowed to send them. Sender's half and the measurements: `~/Code/hoops-sim/docs/milestones/hub-v2.md`.

🔴 **THE DEPLOY ORDER IS THE WHOLE DESIGN.** `checkFeatures` REFUSES a publish carrying an unknown token — it does not degrade, it rejects, and the site keeps serving whatever it had. So this receiver deploys first and hoops-sim's `exporthub.FEATURES` flip is a separate commit cherry-picked afterwards. **Never add a token to hoops-sim's list before the matching `SUPPORTED_FEATURES` entry is LIVE on Railway.** The reverse order takes the hub's data down.

| token | what this receiver does with it |
|---|---|
| `symmetric_off_def` | v1: price a roster edit as `off = net/2`, `def = -net/2` |
| `split_off_def` | v2: price it per side, from `value_off_per36`/`value_def_per36` |
| `depth_chart_minutes` | v2: allocate minutes off the fitted rotation curve, not proportionally |
| `nightly_strength` | v2: prefer the last-ten-games team read when pricing a game |
| `goto_scorer` | v3 (2026-09-06, hoops-sim issue #73): a team built around one big scorer is worth more than its five men summed — add `star_goto_premium × (clamp(leading scorer's share of projected points, star_goto_share_band) − star_goto_league_share)` to a roster's raw strength, the share projected from each man's `pts_per36` over his allocated minutes, summed in ascending order. Net only; the tilt (game totals) never moves. Seven fixture cases (`v3 goto …`), including one that proves the port charges NOTHING when the token is absent. `pts_per36` ships on every player but is not stored in `hoops_players` yet — nothing on the site prices at runtime; the site's team ratings are transported from hoops-sim, which carries the term once its `FEATURES` flips |

`SUPPORTED_PRICING_VERSION` is 3 since the go-to round (was 2). Rule 4 is one-directional, so **every bundle hoops-sim sends today still imports and is still priced the v1 way** — which it will be until the flip lands. Two negotiation rules were added, both about a bundle being ACCEPTED and then priced wrongly (the failure nobody sees on screen): `checkFeatureCoherence` refuses `symmetric_off_def` and `split_off_def` together, and `checkFeatureConstants` refuses a DECLARED feature whose constants are absent while never demanding an undeclared one's (`FEATURE_CONSTANTS`) — which is what keeps a v1 bundle importable forever.

**What changed in `pricing.ts`, in basketball terms.** A player's offence now lands on his new team's offence and his defence on its defence, instead of half his net landing on each — under the old formula every team's two sides were mirror images, so the site projected the same total points for every matchup in the league. And minutes come off the fitted rotation curve rather than each man's own average, because a real rotation is top-heavy: a star arriving in a trade slots near the top of the chart instead of stranded at bench-tier rank. 🔴 The NET is still computed by the untouched lines and is never derived from the two sides, which is what makes it structurally impossible for the split to move a margin or a win probability.

🔴 **The dressed-list cut is deliberately NOT ported and stays server-side.** hoops-sim's own trade pricer (`price_trial_move`), `whatif` and `seasonwhatif` all price an edit through `team_net_rating`, which takes no availability at all — the dressed weighting is applied only by the 30-team baseline pass whose output we transport. Porting it would give this hub a pricing function no hoops-sim surface has, which is exactly the second-implementation trap the fixture exists to prevent.

🔴 **A known non-portable edge, recorded rather than hidden:** hoops-sim's `_depth_chart_rank_key` builds its calibration by sorting a Python SET, whose iteration order for EQUAL values is hash-derived and not reproducible here, so two teammates with exactly equal `value_per36` can order differently on the two sides. Distinct values — every real roster, every fixture case — are byte-for-byte matched. Also: `depthChartBlend` must emit its map in RANK order, because the renormalisation that follows sums floats in iteration order; that one is guarded by a comment, not by the fixture, since a reassociation difference (~1e-15) sits below its 1e-6 tolerance by construction.

**Nightly on `/hoops/teams`.** A fourth lens beside Results/Roster/Blend: each team re-priced from the men who actually played its last ten games, mixed with the season-long results rating. Three refusals that are correctness, not polish — a team with no read is stored **NULL, never 0** (a zero reads as "exactly average lately"); the lens is offered **only when all 30 teams are covered** (`availableRatingModes`, pure, in `rating.ts` so the client and the plain-node build check can both reach it — a half-covered table is two different questions in one column); and the page **names** the teams the read abstained on. The on-screen disclosure says it is a MARGIN read only: a team's contribution to a game's total is carried through from the Results rating unchanged. Whether the site may PRICE a game with it is the sender's call, carried as `nightly_priced`; the block ships either way, so the number is visible before it is trusted. `run_id` gained mode code `n` (position 2 — the court code `n` for neutral is position 1, so they cannot collide) and every historical id still resolves.

**Three build gates**, all appended to `prebuild` AFTER `check:books:discworld` (do not reflow that line): `check:hoops:contract` (negotiation; its first case is the literal v1 contract block and it must sail through), `check:hoops:pricing-fixture` (16 cases in two families, plus eight backward-compatibility pins recorded before hub-v2 so a v1 regression is convicted even if the sender's own numbers moved with it), `check:hoops:nightly` (the real schema, the real importer INSERT, the real rating helpers, and every run_id mode × court combination). Falsification at merge: 10 receiver probes, 10 caught — one only after a REAL HOLE was found and the CHECK repaired (clamping the rank-key interpolation at a single end still passed, i.e. a star arriving better than anyone already on the roster would have been projected for the worst man's minutes; a new fixture case now reaches that branch).

🔴 **Two decision pins in `check-hoops-players.ts` were RE-DECIDED, not softened.** The scope guard used to grep `pricing.ts` for the string `value_off_per36` — a pin that fails the build the day the receiver is legitimately taught the split. It now asserts the BEHAVIOUR (a symmetric bundle prices `tilt` exactly 0 and `off` exactly `net/2` even when every player carries a real split; the same roster under the split prices a non-zero tilt, so "gated off" is distinguishable from "not implemented"; and the split never moves the net) — strictly stronger, since a grep never proved any arithmetic. The absence-tolerance check likewise now asserts coverage when the bundle carries stack ratings, and forces the absent branch by STRIPPING the fields from real rows, so that branch is exercised on every build instead of only while the exporter happened not to ship them.

#### What moved (nightly movers) — 2026-09-02

The nightly rating is one number and one number cannot say why. A team page now names the men behind it: the gap between the nightly read and the SAME roster priced off its season-typical minutes, split one piece per player, biggest three shown. Milwaukee reads 6.6 points a game below its own season-typical price and the block says 3.2 of that is Giannis Antetokounmpo not having played one of the last ten. Sender's half and the measurements: `~/Code/hoops-sim/docs/milestones/nightly-movers.md`.

**The data.** Display fields under each team's `nightly` block, **no wire token** (the `value_off_per36` / explain-sidecar precedent), plus top-level `nightly_league_typical_shift` and `nightly_movers_note`. Here: `nightly_movers_json` + four scalars on `hoops_teams`, `nightly_league_typical_shift` on `hoops_params` (additive migrations), a **sixth** no-op marker in the importer (`hasMovers`, read off the BLOB column so an existing volume backfills once), `getTeamMovers(tri)` and `NightlyMeta.leagueTypicalShift` in `queries.ts`. `getTeamRows()` still returns the raw row; nothing but `getTeamMovers` parses the JSON column.

🔴 **Four numbers the copy must keep apart, and only three of them are related.** `delta_pre_mix` is the team's WHOLE gap and the FULL decomposition sums to it exactly; `movers_delta_sum` is the sum over the three SHOWN, which can legitimately EXCEED the whole gap when the men below cancel (Boston: +1.5 shown against a +1.2 team gap); `delta_post_mix = delta_pre_mix × (1 − results_w)` is the share that survives the mix into the nightly number. `nightly_vs_results` is a FOURTH quantity that these men do not decompose, so the page never quotes it and the read model does not store it.

🔴 **`athlete_id` −1 is the "next man up" absorption slot, not a person** — rendered as dim text, never linked to a player page that cannot exist. 🔴 And `back` is an AVAILABILITY word, not a minutes word: a returning man's delta can be negative if he is below replacement and taking a bigger share (Ousmane Dieng, MIL, back at 32.4 minutes a night against 9.8, −1.40). The league-wide drift (this build, 1.3 points a game lower off ten games than off a season) is printed apart from the movers precisely so it is charged to nobody's name.

Gate: `npm run check:hoops:movers` (appended to `prebuild` at the END, after `check:hoops:matchup`). It reads the importer's own `run(...)` argument list and demands the blob's slot binds the serialised block **by position**; round-trips MIL and BOS through the real schema and the real INSERT, plus a team with no read that must come back NULL; and over the committed bundle asserts the sort, the count, both sums, the mix identity, the abstain rule and the −1 rule. Two REAL-basketball anchors from `espn_player_box` (hoops-sim's, not this bundle's movers): Giannis is Milwaukee's biggest mover, `out`, none of the last ten; Tatum is Boston's, `back` — each clear of the runner-up by more than 1.2 a game, and if either leaves the league the failure says the ANCHOR LIST is stale rather than blaming the decomposition. **11/11 falsification probes caught, one only after a REAL HOLE was found and the CHECK repaired**: the marker assertion grepped the whole file for `moversSettled` and stayed green when the term was deleted from the no-op `if` and left declared above it — it now reads the CONDITION.
