...
- **No refresh button.** Manual refresh is pull-to-refresh on touch and `r` on desktop; the resume-refresh covers the ambient case. Results ("N new items" / "Up to date" / "Refresh failed") ride the shared toast slot with a short auto-dismiss. In-flight feedback depends on the initiator: a pull shows its own indicator zone (see Touch UX), so the "Refreshing…" toast renders only for non-pull refreshes (`pullPhase !== "refreshing"`). The PWA-resume path stays silent on success.
