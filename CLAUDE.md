...
- **Time-based falloff** (`UNREAD_TTL_HOURS` in `lib/queries.ts`). After every poll cycle, `pruneExpiredUnread` marks items older than their category's TTL as read. RSS categories (music, books, film, podcasts, reading) have a 7-day TTL. Bluesky has **no TTL** — it doesn't need one because the All view only pulls enough bsky posts to interleave (see below), and the bluesky tab shows all unread posts via pure recency. `consumed_at` is NEVER set by the prune, so /read history is unaffected. Pruned items are "pending": still in `items`, with `item_state.read_at` set, invisible to every view.
- **All view = every unread RSS item + bluesky sprinkled in.** `getMainFeedItems` fetches all unread items from each RSS category (music, books, film, podcasts, reading) with no cap — the TTL prune is the only bound. Bluesky is derived from the RSS total: `1 bsky per BSKY_INTERLEAVE_RATIO (4) RSS items`, with a minimum of 10 and a fallback of `BSKY_FALLBACK_MAX (100)` when there are no RSS items. The bsky posts are selected via surprise sampling (`selectWithSurprise` using `SURPRISE_POOL_MULTIPLIER` and `SURPRISE_RECENCY_BIAS`) for variety within the newest posts.
