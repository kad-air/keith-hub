...
- **Per-setlist cache warming.** The library no longer warms every chart (that was the old single-setlist behavior). Only setlists with `offline = 1` warm their member chart pages: `SetlistDetailClient` warms a setlist's charts when its toggle is on, and `ChartsClient` warms all offline-flagged setlists' pages on load (covers cold PWA relaunch — `getOfflineSetlists()` feeds it the chart ids). Each warms by `fetch()`ing the page (covers cold relaunch / shared link) and `router.prefetch()`ing the RSC payload (covers in-app `<Link>` nav); both land in `charts-pages`. Warming is keyed on the member set and guarded by a ref so it runs at most once per unchanged set per session. Status line: "Saving for offline…" → "✓ Saved for offline", or "● Offline · charts available" when disconnected. Writes (add/edit/delete/reorder/rename/toggle) still require the network — offline is read/play only.
