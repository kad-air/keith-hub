...
- **All view = quota-based sampling**, not inclusion or ranking. `ALL_VIEW_QUOTAS` in `lib/queries.ts` defines a per-category quota; `getMainFeedItems` takes the most recent N unread items from each category (where N = quota for that cat), merges them, and sorts by `published_at DESC` for display. Sum of quotas = the All view's max size (currently 300: `podcasts 100, music 50, film 50, reading 50, bluesky 50`). High-signal items are NOT pinned to the top — they're guaranteed representation by the quota then mixed in chronologically. Tunable; the constants are at the top of `lib/queries.ts`.
